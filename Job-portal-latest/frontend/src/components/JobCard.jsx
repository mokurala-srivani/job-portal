...
export default function JobCard({ job, onSave, saved }) {
  ...
  return (
    <motion.article
      ...
      className="group relative flex flex-col gap-4 overflow-hidden rounded-2xl border border-slate-200 bg-white/80 p-5 backdrop-blur-xl shadow-soft transition hover:border-brand-300 hover:shadow-glow"
    >
      <span className="pointer-events-none absolute -right-16 -top-16 h-40 w-40 rounded-full bg-brand-300/0 blur-2xl transition-all duration-500 group-hover:bg-brand-300/30" />

      <div className="relative flex items-start gap-3">
        <CompanyLogo name={job.company} size={48} />
        <div className="min-w-0 flex-1">
          <Link to={`/jobs/${job._id}`} className="block truncate font-display text-base font-bold text-strong group-hover:text-brand-700">
            {job.title}
          </Link>
      ...
      <div className="relative mt-auto flex items-center justify-between border-t border-slate-100 pt-3">
